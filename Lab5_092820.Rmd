...
#EXERCISE 5
```{r}
Deaths_3_13 <- Deaths_3_13 <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/03-13-2020.csv")) %>%
  rename(Country_Region = "Country/Region", Province_State = "Province/State") %>% 
  filter (Country_Region == "US") %>%
  group_by(Province_State, Country_Region) %>%
...
ggplot(x = Date, y = Cofirmed_Deaths_Ratio) + 
geom_point() + 
geom_line()
```
#EXERCISE 7 ** does not give correct table after arrange and slice
```{r}
...
geom_bar() 
```
#EXERCISE 8** does not work for same reason as Exercise 7, also Country_Region not found in facet_wrap
```{r, fig.width = 10}
top10_countries_deaths <- arrange(time_series_deaths_long, 
desc(Deaths)) %>%
slice(1:10) %>%
...
```
#EXERCISE 9 **object Province_State not found 
```{r, fig.width = 10}
...
geom_line()
```
#EXERCISE 10 ** Does not work because Exercise 8 did not work
```{r, fig.width = 10}
top10_countries_deaths <- arrange(time_series_deaths_long, 
desc(Deaths)) %>%
slice(1:10) %>%
